...
# --- Helper function to compute similarity between two tuples ---
...
def method2(input_file, T, output_file, queries):
    ...
    # --- Compute popularity just like Method 1 ---
    ...
    # Collecting to compute similarity matrix (for small datasets)
    data = df.collect()
    num_rows = len(data)
    sim_matrix = [[0.0 for _ in range(num_rows)] for _ in range(num_rows)]

    # --- Computing similarity for each pair of tuples ---
    for i in range(num_rows):
        for j in range(i + 1, num_rows):
            s = tuple_similarity(data[i], data[j])
            sim_matrix[i][j] = sim_matrix[j][i] = s

    # --- Compute weighted importance per tuple ---
    importance_values = []
    for i in range(num_rows):
