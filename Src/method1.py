...
def method1(spark, input_file, T, output_file, queries):
    ...
    df_pop = pop_method1(df, queries)
    df_topT = df_pop.limit(T)

    end_time = time.time()
    
    # Sum of total hits in R and R'
    imp_R = df_pop.agg({'total_hits': 'sum'}).collect()[0][0]
    imp_R_prime = df_topT.agg({'total_hits': 'sum'}).collect()[0][0]
